    ...
    <div className="overflow-x-auto rounded-xl shadow border border-slate-200 bg-white">
      <table className="w-full text-sm text-left text-slate-600">
        <thead className="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200">
          <tr>
            <th scope="col" className="px-6 py-4 font-bold border-r border-slate-200">
              Action Model
            </th>
            <th scope="col" colSpan={4} className="px-6 py-4 text-center font-bold border-r border-slate-200 bg-blue-50/50 text-blue-900">
              AVS-ProcTHOR (Synthetic)
            </th>
            <th scope="col" colSpan={6} className="px-6 py-4 text-center font-bold bg-green-50/50 text-green-900">
              AVS-HM3D (Real)
            </th>
          </tr>
          ...
          {MAIN_RESULTS_DATA.map((row, index) => (
            <tr 
              ...
              className={`border-b border-slate-100 last:border-none hover:bg-slate-50 transition-colors ${
                row.highlight ? 'bg-blue-50/20 font-medium' : ''
              }`}
            >
